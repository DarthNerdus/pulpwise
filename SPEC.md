...
- **Status:** v0.1.0 (beta). Spec phases 0-3 complete; post-MVP roadmap items shipped: Substack source + bulk importer (cookie auth), arXiv (PDF passthrough), MangaDex (CBZ + read-from-start), OPML import, TUI (Library/Subscriptions/Sync/Stats), syncthing CLI integration, CLI progress bars during sync, Anna's Archive search + download (donation API), email source (IMAP mailbox → attachments + newsletter EPUBs), RSS category filtering (`categories`/`exclude_categories` subscription options), dedicated output dir for Anna's downloads (`[auth.annas].output_dir`). GitHub Actions CI configured. PyPI publish staged but not yet executed.
...
- RSS category filtering is per-subscription options (`categories` allow-list / `exclude_categories` drop-list, comma-separated strings because option values are `str | int`), matched case-insensitively against feedparser's normalized `entry.tags[].term` and applied in `discover()` - filtered entries never reach `fetch` or the ledger, so loosening the filter later ingests whatever is still in the feed. An allow-list skips untagged entries; exclusion wins over inclusion.
- Anna's download destination lives in `[auth.annas].output_dir`, not a new config section: `[auth.annas]` is already the de-facto annas settings table (`mirrors` precedent), and a plain string value passes the generic auth validation, so the core schema is untouched. Mechanism is a generic `Source.output_dir` instance attribute set by the plugin in `from_config` and consulted by `add_once` when building the sink; it beats the base dir (explicit `output_dir` arg included) because it is the user naming a destination for that source specifically. One-shot base dir precedence: explicit arg > `PULPLINE_OUTPUT_DIR` env > config `paths.output_dir` > hardcoded default (the config fallback was previously ignored by `add_once`).
