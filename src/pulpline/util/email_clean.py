"""Newsletter-email HTML cleanup for e-reader rendering.
...
# deliberate: e-ink readers do better with their own defaults than with
# newsletter-tool inline CSS.
...
# Elements that never carry content in an email (or that an EPUB reader
# can't render safely: frames, forms, embedded players).
_DROP_ELEMENTS = (
    "script",
    "style",
    "link",
...
def clean_email_html(
    ...
    """Return a cleaned HTML fragment suitable for the EPUB renderer.

    `inline_images` maps Content-ID (without angle brackets) to
    `(content_type, bytes)`; matching `cid:` image references are rewritten
    to data: URIs, which the renderer then embeds as EPUB resources.
    Raises ExtractionError when the input has no usable content.
    """
...
def _is_tracking_pixel(img: lxml.html.HtmlElement, src: str) -> bool:
    ...
    # still present here; it's stripped later in the pipeline, and the EPUB
    # renderer would fetch the tracker at render time if we kept the img.
