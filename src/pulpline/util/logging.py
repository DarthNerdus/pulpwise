"""File-based logging at `~/.local/state/pulpline/log/pulpline.log`.

Default behavior: log to file only; stderr stays clean. `pulp sync` runs
under cron benefit from durable logs even when their stderr is captured
into mail or `/dev/null`. INFO level by default; DEBUG when `--verbose`
is passed at the CLI.

The shape of `pulpline.<module>` loggers mirrors the package layout, so
`get_logger("pipeline")` writes records tagged `pulpline.pipeline`.
...
LOGGER_NAME = "pulpline"
DEFAULT_FILENAME = "pulpline.log"
...
def default_log_dir() -> Path:
    """XDG state dir for logs. `PULPLINE_LOG_DIR` env var overrides."""
    override = os.environ.get("PULPLINE_LOG_DIR")
    if override:
        return Path(override).expanduser()
    return Path.home() / ".local" / "state" / "pulpline" / "log"


def setup_logging(verbose: bool = False, log_dir: Path | None = None) -> Path:
    """Wire up file logging on the `pulpline` logger.

    Idempotent: re-calling clears the previously installed pulpline
    ...
    for existing in list(logger.handlers):
        if getattr(existing, "_pulpline_owned", False):
    ...
    file_handler._pulpline_owned = True  # type: ignore[attr-defined]
    logger.addHandler(file_handler)

    if verbose:
        stderr_handler = logging.StreamHandler()
        stderr_handler.setFormatter(logging.Formatter(_STDERR_FORMAT))
        stderr_handler._pulpline_owned = True  # type: ignore[attr-defined]
...
def get_logger(module: str) -> logging.Logger:
    """Return a child logger of `pulpline.<module>`."""
