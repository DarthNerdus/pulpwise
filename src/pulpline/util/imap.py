...
def parse_mailbox_url(url: str) -> MailboxLocation:
    ...
    The path is the IMAP folder (URL-escaped, so `Pulpline%20Inbox` works);
...
class ImapSession:
    ...
    def find_uid_by_message_id(self, message_id: str) -> str | None:
        """Locate a message by Message-ID header. Used when fetch() runs
        without a prior discover() in this session (`pulp migrate --rebuild`)."""
        try:
